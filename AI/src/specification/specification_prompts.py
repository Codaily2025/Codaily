...
- 'spec': 사용자가 만들고자 하는 서비스 또는 프로젝트, 플랫폼, 게임 등에 대해 정보를 제공한 경우 (명세서 생성 가능)
...
2) 그 외에는 'spec' 판단 규칙을 적용한다.
3. 그 외에는 'spec:regenerate' 판단 규칙을 적용한다.
4) 그 외에 'spec:add:field' 판단 규칙을 적용한다.
5) 그 외에 'spec:add:feature:sub' 판단 규칙을 적용한다.
6) 그 외에 'spec:add:feature:main' 판단 규칙을 적용한다.
7) 그 외에는 'chat', 'chat:smalltalk', 'ignore:drop' 등으로 판단한다.


다음 조건 중 **1개 이상을 충족**할 경우에만 'spec'으로 판단하세요:
1. 서비스 목적이나 해결하고자 하는 문제 설명
2. 명확한 핵심 기능명 ('로그인', '알림 설정' 등)
3. 사용 흐름/시나리오 ('이메일을 입력하면 인증 링크 전송')
4. 입력값과 출력값의 관계 ('입력하면 요약된 정보 반환')
5. 명세 생성 의사 표현 (프로젝트/ 서비스/ 게임/ 등등 '에 대한 명세서 만들어줘, 만들고 싶어, 진행하고 싶어 등')
6. 챗봇이 명세 생성 안내를 먼저 한 경우
7. 사용자의 발화가 서비스 전체 또는 신규 프로젝트 아이디어를 설명하는 경우
  (예: "비행기 탄막 게임을 만들고 싶어요", "반려동물 관리 앱을 만들 계획이에요")
...
- 사용자 문장에 재생성 의도를 나타내는 명시적 표현(정규식: /(다시|재생성|전면|처음부터|초기화|리셋)/)이 포함
- '명세서 생성/작성/만들어줘'와 같은 일반 표현만 있는 경우는 항상 'spec'입니다.
...
- 특정 기능 그룹과 관련이 있다면 반드시 field에 해당 기능 그룹을 넣으세요.
...
단, 사용자의 요구 사항을 무조건 무조건 무조건 지키세요.
"""

SPEC_GEN_FIELD_USER = """
**다음 조건을 정확히 따라 기능 그룹을 도출하세요.**\n
- 항목 개수: **{min}개 이상, {max}개 이하**
- 각 항목의 시간 합계는 반드시 **{{time}}시간 이하여야 하며**, 정확히 {{time}}시간이 되도록 분배하세요.
- 기능 그룹명은 명사의 형태로 출력합니다.
- 만약 {{time}}이 0이라면, 절대 아무 기능 그룹이나 항목도 생성하지 말고, 완전히 빈 출력만 하세요. 
  이 경우는 예외 없이 반드시 빈 출력이어야 합니다.
- 각 기능 그룹의 시간은 최소한 0.5부터 시작해야 합니다. 만약 시간 초과 등으로 0.0이 되어야 한다면 그 그룹은 출력물에서 제외하세요.
- **각 '기능 그룹'의 시간 합계가 {{time}}시간을 초과하면, 그 기능 그룹 전체를 절대 출력하지 마세요.**
- 초과하는 기능 그룹이 있다면 해당 그룹은 아예 생성하지 말고, 출력물에서 제외하세요.
- **하나라도 초과하는 그룹이 있으면 그 그룹은 무조건 삭제해야 하며, 일부 항목만 빼서 조정하지 마세요.**
- 조건을 만족하는 기능 그룹이 하나도 없으면 아무것도 출력하지 마세요. (빈 출력 허용)
- 시간 분배는 정확히 0.5시간 단위로 해야 합니다. 0.5, 1.0, 1.5, 2.0, ...
- 형식은 아래의 형식을 따르고, 다른 대화나 양식, 항목명 같은 것들은 절대 추가하지 마세요.
- 형식: `- 기능 그룹명:시간` (설명이나 번호 없이)
- 각 항목의 시간은 양수로 표기하세요.
- 형식의 예시는 다음과 같습니다.
    - 사용자 관리:1.5
    - 결제 처리:1.0
...
- 시간 분배 규칙:\n
  1) 각 항목의 시간 합계는 반드시 **{{time}}시간 이하여야 하며**, 정확히 {{time}}시간이 되도록 분배하세요.\n
  2) 시간 분배는 정확히 0.5시간 단위로 해야 합니다. 0.5, 1.0, 1.5, 2.0, ... (0.0 안됨)\n
  3) 반올림 등으로 총합이 어긋나지 않도록 각 항목 시간 합계 <= {{time}}로 맞춤\n
...
- 시간 제약(아주 중요):\n
  1) 각 항목의 시간 합계는 반드시, 무조건 **{{time}}시간 이하**가 되도록 분배하세요.
  2) 시간 분배는 정확히 0.5시간 단위로 해야 합니다. 0.5, 1.0, 1.5, 2.0, ... (0.0 안됨)\n
  3) 각 항목의 예상시간 총합이 어긋나지 않도록 **정확히, 무조건 {{time}}시간 이하**로 맞춤\n
- 개수: **{min}개 이상, {max}개 이하**\n
- 우선순위 규칙: **정수 1~10** (1이 가장 높음), 동점 최소화\n
- 금지: 추가 문장/번호/코드블록/단위(h 등), 범위값(예: 1-2), '~', '약/대략' 같은 수식\n
- 출력은 **아래 bullet 리스트만**. 그 외 텍스트 출력 금지.\n\n
[검증 단계 — 출력 전에 스스로 점검]:
- 상세기능들의 예상시간 총합이 {{time}}이 아니면, 항목 수는 유지하되 예상시간을 0.5 단위로 증감하여 **총합을 정확히 {{time}}시간 이하**로 조정한다.
- 조정 시, 우선 우선순위가 낮고 시간이 큰 항목부터 0.5씩 조정하고, 상세기능들의 예상시간 총합이 {{time}}이 될 때까지 반복한다.
【예시】
- 이메일 입력 필드 표시: 사용자가 이메일을 입력할 수 있는 입력창을 화면에 표시: 0.5: 8
- 가입 버튼 클릭 처리: 사용자가 가입 버튼을 클릭하면 계정 생성 요청을 서버에 전송: 1.0: 10
- 비밀번호 확인 검증: 비밀번호와 비밀번호 확인 값이 같은지 실시간으로 검증: 0.5: 7
▼ 프로젝트 설명:\n
{{description}}\n\n
▼ 기능 그룹:\n
{{group}}\n\n
...
- 기능명은 가능한 한 간결하고 구체적인 **명사형 표현**으로 작성하세요\n
- 설명은 **사용자 관점**에서, 이 기능을 통해 **무엇을 할 수 있는지 한 줄로** 작성하세요\n
- 각 항목의 예상시간은 **0.5 단위의 양수**, **0.5 단위**로 표기\n
- 형식 오류나 누락이 없도록 주의하세요\n
...
- 상세기능명: 설명: 예상시간(0.5 단위): 우선순위(1~10 정수)
...
3. 예상 시간은 **0.5~10.0 범위 양수**, **0.5 단위**로 표기하며 단위는 시간입니다.
...
SPEC_ADD_GROUP_SYS = """
당신은 사용자의 요구로부터 적절한 소프트웨어 기능 분류 이름과 예상 소요 시간을 하나 도출하는 전문가입니다.\n
출력 형식은 반드시 **'기능명:시간'**이어야 합니다. 순서는 반드시 기능명, 시간 순서여야 합니다.\n
- 기능명: 하나의 단어 또는 짧은 문장
- 시간: 예상 소요 시간(시간 단위 양수), 소수 1자리까지 0.5 단위로 표기
- 예시:
